...
# Determine 3' UTR intervals per transcript based on strand
three_utr_regions = []
for tid, utrs in utr_by_transcript.items():
    ...
    # Determine the 3' end of CDS for this transcript
    if strand == "+":
        cds_3prime = max(e[1] for e in cds_intervals)
        # 3' UTR is UTR region **after** CDS end
        three_prime_utrs = [(start, end) for start, end in utrs if start > cds_3prime]
    else:  # strand == "-"
        cds_3prime = min(e[0] for e in cds_intervals)
        # 3' UTR is UTR region **before** CDS start (lower coordinates)
        three_prime_utrs = [(start, end) for start, end in utrs if end < cds_3prime]

    for start, end in three_prime_utrs:
        three_utr_regions.append((chrom, start, end, strand, tid))

# Now check which SNPs fall in 3' UTR regions

def snp_in_region(snp_pos, regions):
...
for idx, snp in snps.iterrows():
    chrom = snp["chrom"]
    pos = snp["start"]
    for region in three_utr_regions:
...
df_results.to_csv("snps_in_3UTR.tsv", sep="\t", index=False)

print(f"Identified {len(df_results)} SNPs in 3' UTR regions.")